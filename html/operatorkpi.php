...
/* Month-to-Date aggregate scrap by operator. Need to aggregate with operator Month-to-Date footage.
SELECT
    created_by_user,
    COUNT(*) AS scrap_records,
    SUM(scrap_lbs) AS total_scrap_lbs,
    SUM(scrap_quantity) AS total_scrap_quantity,
    SUM(scrap_ext) AS total_scrap_ext
FROM pwo_scraps
WHERE created_date_time >= DATE_FORMAT(CURDATE(), '%Y-%m-01')
  AND scrap_lbs <> 0
GROUP BY created_by_user
ORDER BY total_scrap_lbs DESC;
